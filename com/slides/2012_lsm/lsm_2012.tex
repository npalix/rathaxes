...
\begin{frame}{State of the Driver}
\begin{center}
\only<1>{\LARGE{Often \cemph{\Huge{instables}} and \cemph{\Huge{buggy}}}}
\only<2>{%
...
\begin{center}
\only<1>{\Large{A \cemph{\LARGE{hard}} task, \cemph{\LARGE{long}} and \cemph{\LARGE{boring}}}}
\end{center}
\only<2>{\Large{%
In cause:
\begin{itemize}
...
\begin{frame}{What \rtx\ offers to answer those issues}
\begin{center}
\only<1>{\Large{\cemph{\LARGE{Simplify}} the development cycles}}
\end{center}
\only<2>{\Large{
To be done through:
\begin{itemize}
\item Abstracting the platforms ;
\item Static checks ;
\item Separated skills.
\end{itemize}}}
\end{frame}

\begin{frame}{Thus the tools for \rtx}
	\begin{center}
        \Large{%
	\begin{itemize}
...
\node[redbox, minimum height=6cm, minimum width=15mm, right=4mm] (BUS) at (SYSTEM.east) {};
    ...
    \node[graybox, below=2mm of ISA] (I2C) at (ISA.south)  {I\textsuperscript{2}C};
...
\begin{frame}{The \rtx\ language (Middle-End)}
\begin{tikzpicture}[overlay]
\node (RTI) at (1.2,0) {\includegraphics[height=1.5cm]{icons/rti}};

\draw (RTI.north east) node[below right]{%
    \begin{minipage}{10cm}
    Describe a \emph{«~subsystem~»} with:
    \begin{itemize}
    \item Types ;
    \item Sequences ;
    \item Variables of configuration.
    \end{itemize}
    \end{minipage}
};
\end{tikzpicture}
...
\begin{frame}[fragile]{The \rtx\ language (Middle-End)}
\begin{tikzpicture}[overlay]
\node (RTI) at (1.2,0) {\includegraphics[height=1.5cm]{icons/rti}};
...
\begin{frame}{The \rtx\ language (Back-End)}
\begin{tikzpicture}[overlay]
\node (BLT) at (1.2,0) {\includegraphics[height=1.5cm]{icons/blt}};

\draw (BLT.north east) node[below right]{%
    \begin{minipage}{10cm}
        Implements the interfaces for each OS:
        \begin{itemize}
        \item Type templates ;
        \item Sequence templates.
        \end{itemize}
    \end{minipage}
};
\end{tikzpicture}
...
\begin{frame}[fragile]{The \rtx\ language (Back-End)}
\begin{tikzpicture}[overlay]
\node (BLT) at (1.2,0) {\includegraphics[height=1.5cm]{icons/blt}};
...
\begin{frame}{The \rtx\ language (Front-End)}
\begin{tikzpicture}[overlay]
\node (RTX) at (1.2,0) {\includegraphics[height=1.5cm]{icons/rtx}};

\draw (RTX.north east) node[below right]{%
    \begin{minipage}{10cm}
        Describes the hardware, the driver's algorithms, and the target environment:
        \begin{itemize}
        \item Registers ;
...
\begin{frame}[fragile]{Interactions between language parts}
\begin{tikzpicture}[overlay]
...
\begin{frame}{Generating a driver}
...
\draw (RTX.north east) node[below right]{\large{Description of the driver}};
...
\draw (RTX.north east) node[below right, opacity=0.3]{\large{Description of the driver}};
...
\draw (BLT.east) node[above right]{\Large{Templates of code}};
...
\begin{frame}{First step into \rtx}
	\begin{center}
...
\begin{frame}{Current situation of \rtx}
\only<1>{
    \Large{%
        A bit of history:
        \begin{itemize}
            \item 2002: Thesis of L. Reveillere \emph{«~Devil~»} ;
            \item 2007: Beginning of the project at LSE (with Epitech);
            \item 2012: Refactoring, gone open-source.
        \end{itemize}
    }
}
\only<2>{
    \Large{%
        ...
        \begin{itemize}
            ...
            \item Coverage of the problematique: ~70\% ;
        \end{itemize}
    }
}
\only<3>{
    \Large{%
        What we need:
        \begin{itemize}
            \item Feedback about the language ;
            \item More input from hardware specialists ;
            \item Contributors.
        \end{itemize}
    }
}
\only<4>{
    \Large{
        In the near future:
        \begin{itemize}
            \item Work closely with small experimental/exotic OSes ;
...
\begin{frame}{Next steps}
	...
	\begin{itemize}
		\item	Fosdem 2013;
		\item	Evolution to Python $\rightarrow$ contributions.
	\end{itemize}
        }
	\end{center}
\end{frame}
